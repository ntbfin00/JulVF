...
"""
function underdense_cells(delta::Array{<:AbstractFloat,3},threshold::Float32,dims::Array{Int64,1},dims23::Int64,in_void::Array{Int8,3},delta_v::Array{Float32,1},IDs::Array{Int64,1})
...
"""
function void_overlap_frac(R_cand::Float32,R_void::Float32,dist2::Int32)
...
"""
function nearby_voids1(voids_total::Int32,dims::Array{Int64,1},middle::Array{Int64,1},i::Int64,j::Int64,k::Int64,void_radius::Array{Float32,1},void_pos::Array{Int32,2},R_grid::Float32,max_overlap_frac::Float32,bound_conds::Bool)

    overlap_frac::Float32 = 0
...
"""
function nearby_voids2(Ncells::Int32,dims::Array{Int64,1},i::Int64,j::Int64,k::Int64,R_grid::Float32,R_grid2::Float32,in_void::Array{Int8,3},max_overlap_frac::Float32,bound_conds::Bool)
...
function nearby_voids2(Ncells::Int32,dims::Array{Int64,1},i::Int64,j::Int64,k::Int64,R_grid::Float32,R_grid2::Float32,in_void::Array{Int8,3},max_overlap_frac::Float32,bound_conds::Bool,threading::Bool)
...
"""
function mark_void_region!(Ncells::Int32,dims::Array{Int64,1},i::Int64,j::Int64,k::Int64,R_grid2::Float32,in_void::Array{Int8,3}, bound_conds::Bool)
...
function mark_void_region!(Ncells::Int32,dims::Array{Int64,1},i::Int64,j::Int64,k::Int64,R_grid2::Float32,in_void::Array{Int8,3},bound_conds::Bool,threading::Bool)
...
"""
function voidfinder(delta::Array{<:AbstractFloat,3}, box_length::Array{<:AbstractFloat,1}, box_centre::Array{<:AbstractFloat,1}, par::Main.VoidParameters.SphericalVoidParams; bound_conds = true, volume = prod(box_length), mask = nothing)
    ...
    voids_max = floor(Int32, box_vol/vol_eff)
    @debug "Maximum number of voids = $voids_max"

    # initialize arrays
    void_pos = Array{Int32}(undef,voids_max,3)  # void positions
    void_radius = Array{Float32}(undef,voids_max)  # void radii

    in_void = zeros(Int8,dims...)  # void membership cell flag
    delta_v = Array{Float32}(undef,cells_total)  # underdense cells density contrasts 
    ...
    @inbounds for (q,R) in enumerate(Radii)  
        ...
        if !threading
            delta_sm = smoothing(delta, dims, R, res)
        else
            delta_sm = smoothing(delta, dims, R, res, threading)
        end
...
"""
function voidfinder(cat::Main.VoidParameters.GalaxyCatalogue, mesh::Main.VoidParameters.MeshParams, par::Main.VoidParameters.SphericalVoidParams)
    ...
    if mesh.nbins_vf == [0]
        mesh.nbins_vf = nbins
        @info "Default voidfinding bins set" 
    end

    delta, mask, box_length, box_centre = create_mesh(cat, mesh; threshold = threshold)

    voidfinder(delta, box_length, box_centre, par; bound_conds = box_like, volume = vol, mask = mask)
